...
1. Call `list_operations_projects` when a request names a project, needs project discovery, or will assign or filter tasks by project or section. It returns each project's sorted `member_usernames` and ordered sections; use only a returned `project_id`, `project_section_id`, and username for the selected account.
2. Use `list_operations_tasks` for task requests. It defaults to every active task the authenticated user can read, including later work; use `status`, `due_group`, `assignee_username`, `contact_padma_id`, `project_id`, or `project_section_id` only when the request needs that filter. Omit either ID to include every project or section; send explicit `null` to return only tasks without that project or section.
